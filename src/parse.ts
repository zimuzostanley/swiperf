// src/parse.ts — Pure parsing functions, no side effects
...
export function parseJsonToTraces(
  ...
  if (Array.isArray(parsed) && parsed.length) {
    ...
    if (
      ...
      parsed.length >= 2 &&
      parsed[0].every((h: any) => typeof h === 'string')
    ) {
    ...
    const looksLikeSlice =
      DEFAULT_SLICE_FIELD_CONFIG.ts.aliases.some(a => first[a] !== undefined) &&
    ...
    if (looksLikeSlice && !looksLikeTrace) {
      ...
      const slices = items.map((s: any) => normalizeSlice(s))
      return [
        {
    ...
    if (looksLikeTrace) {
      ...
      for (let i = 0; i < total; i++) {
        ...
        const t = normalizeTrace(items[i])
        if (t) traces.push(t)
      }
...
export function parseDelimitedToTraces(
  ...
  if (slicesIdx < 0) {
    throw new Error(
      `Need a column matching: ${cfg.slices.aliases.join(', ')}`,
    )
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    if ((i - 1) % 10 === 0) {
      onProgress?.({
        message: `Processing row ${i}/${total}...`,
        current: i - 1,
        total,
      })
    }
    ...
    try {
      ...
      const sliceArr = Array.isArray(sliceParsed)
        ? sliceParsed
        : sliceParsed.slices ?? sliceParsed.data
      ...
      if (pkgName.startsWith('{')) {
        try {
          const p = JSON.parse(pkgName)
          if (p.package_name) pkgName = p.package_name
        } catch { /* not JSON */ }
      }
